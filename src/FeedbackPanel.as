package
{
	...
	public class FeedbackPanel extends Panel
	{
		...
		public var feedbacks:Vector.<MyoFeedback>;
		private var feedbackContainer:Sprite;
		
		private var _selectedFeedback:MyoFeedback;
		...
		public function FeedbackPanel()
		{
			...
			feedbacks = new Vector.<MyoFeedback>;
		...
		private function placeElements():void
		{
			...
			{
				var f:MyoFeedback = feedbacks[i];
				var angle:Number = i * Math.PI * 2 / feedbacks.length;
				var dist:Number = INNER_RADIUS + MyoFeedback.RADIUS + 5;
		...
		private function showMyoInfos(feedback:MyoFeedback):void 
		{
			TweenLite.to(infos, .3, { alpha:feedback != null?1:0 } );
			TweenLite.to(logo, .3,  { alpha:feedback != null?0:1 } );
			
			if (feedback == null) return;
			infosTF.text = selectedFeedback.myo.id + "\nDevice is " + (selectedFeedback.myo.isTrained?"trained":"not trained");
		...
		{
			for each(var m:MyoFeedback in feedbacks)
			{
				m.myo.vibrate(intensity);
			}
		}
		...
		public function vibrateMyo(myoID:String, intensity:int  = Myo.VIBRATION_SHORT):void
		{
			for each(var m:MyoFeedback in feedbacks)
			{
				if (m.myo.id == myoID) m.myo.vibrate(intensity);
			}
		}
		...
		{
			selectedFeedback = e.target as MyoFeedback;
		}
		...
		public function get selectedFeedback():MyoFeedback 
		{
			return _selectedFeedback;
		}
		
		public function set selectedFeedback(value:MyoFeedback):void 
		{
